...
//        System.out.println("Precio por hora "+valueHour+" horas transcurridas "+hours+" y el total es "+valueHour*hours);
    ...
    public PageDTO<List<ReservationPaymentDTO>> getPaymentsByUserConsole(Map<String, Object> queryParams) {
        ...
        Page<ReservationPayment> reservationPayments;
        if(params.getSearch()!=null){
            System.out.println(Long.parseLong(params.getSearch()));
            reservationPayments = reservationPaymentRepository.findAllByOwnerIdAndSearch(params.getUser(), Long.parseLong(params.getSearch()), pageable);
        }else{
            reservationPayments = reservationPaymentRepository.findAllByOwnerId(params.getUser(), pageable);
        }
